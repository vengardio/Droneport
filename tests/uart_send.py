"""
Консольная утилита: отправка сырых пакетов на STM32 через UART.

Протокол: [0xBF | TYPE | LEN | DATA | CRC | 0xFF]
CRC = (TYPE + LEN + sum(DATA)) & 0xFF

Использование:
    python uart_send.py                        # порт /dev/ttyS0, 115200
    python uart_send.py --port COM3 --baud 9600

В консоли вводишь TYPE-байт и DATA в hex, утилита сама считает LEN и CRC,
собирает пакет, показывает его в двоичном виде и отправляет.
"""
...
def fmt_binary(raw: bytes) -> str:
    """Каждый байт в формате 0bXXXXXXXX (0xHH)."""
    parts = []
    for b in raw:
